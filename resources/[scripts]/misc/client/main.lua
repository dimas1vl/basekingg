...
function DisableAllPickups()
	local playerId = PlayerId()
	for i = 1, #pickups do
		ToggleUsePickupsForPlayer(playerId, pickups[i], false)
		RemoveAllPickupsOfType(pickups[i])
	end
end

CreateThread(function()
	AddTextEntry("FE_THDR_GTAO", "KinGG")
	...
	StartAudioScene("MUTES_AMBIENCE_SCENE")
	SetAmbientZoneState("0", false, false)
	SetAmbientZoneStatePersistent("AZ_DISTANT_SASQUATCH", false, false)
	ClearAmbientZoneState("AZ_COUNTRYSIDE_PRISON_01_ANNOUNCER_GENERAL", true)
	ClearAmbientZoneState("AZ_COUNTRYSIDE_PRISON_01_ANNOUNCER_WARNING", true)
	ClearAmbientZoneState("AZ_COUNTRYSIDE_PRISON_01_ANNOUNCER_ALARM", true)
	...
	while true do
		...
		local wpn = GetSelectedPedWeapon(PlayerPedId())
		local grp = GetWeapontypeGroup(wpn)
		if grp ~= `GROUP_MELEE` and grp ~= `GROUP_UNARMED` then
...
-- AGACHAR (crouch) — toggle no CTRL via keymapping
local crouched = false
local CROUCH_CLIPSET = "move_ped_crouched"
local CROUCH_STRAFE = "move_ped_crouched_strafing"
...
local function setCrouch(state)
	...
	if state then
		...
		while not HasAnimSetLoaded(CROUCH_CLIPSET) and GetGameTimer() < deadline do
			Wait(0)
		end
		if not HasAnimSetLoaded(CROUCH_CLIPSET) then
			return
		end
		SetPedMovementClipset(ped, CROUCH_CLIPSET, 0.25)
		if HasAnimSetLoaded(CROUCH_STRAFE) then
			SetPedStrafeClipset(ped, CROUCH_STRAFE)
		end
		crouched = true
	else
...
RegisterCommand("misc:crouch", function()
	local ped = PlayerPedId()
...
RegisterKeyMapping("misc:crouch", "Agachar", "keyboard", "LCONTROL")

CreateThread(function()
	while true do
		local wait = 1000
		local ped = PlayerPedId()
		if ped ~= 0 and not IsPedInAnyVehicle(ped, false) and not IsEntityDead(ped) then
			if crouched then
				wait = 0
...
CreateThread(function()
	...
	while true do
		...
		if crouched then
			if inVeh or IsEntityDead(ped) then
				setCrouch(false)
			else
				if not HasAnimSetLoaded(CROUCH_CLIPSET) then
					RequestAnimSet(CROUCH_CLIPSET)
				else
					if lastCrouched ~= crouched then
						SetPedMovementClipset(ped, CROUCH_CLIPSET, 1.0)
					end
				end

				if not HasAnimSetLoaded(CROUCH_STRAFE) then
					RequestAnimSet(CROUCH_STRAFE)
				else
					if lastCrouched ~= crouched then
						SetPedStrafeClipset(ped, CROUCH_STRAFE)
...
CreateThread(function()
	while true do
		...
		ClearAreaOfVehicles(0.0, 0.0, 0.0, 10000, false, false, false, false, false)
		RemoveVehiclesFromGeneratorsInArea(-10000.0, -10000.0, -10000.0, 10000.0, 10000.0, 10000.0)
