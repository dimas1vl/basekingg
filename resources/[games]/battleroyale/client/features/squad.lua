...
local function getMemberName(src)
	if gNameCache[src] then
		return gNameCache[src]
	end
...
local function isSquadMember(serverId)
	if not Game.session.squad or not serverId then
		return false
	end
...
local function rayCastCamera()
	...
	local target = vector3(coord.x + dir.x * dist, coord.y + dir.y * dist, coord.z + dir.z * dist)

	local rayHandle =
		StartShapeTestLosProbe(coord.x, coord.y, coord.z, target.x, target.y, target.z, -1, PlayerPedId(), 1)
...
local function emitPing()
	...
	if Game.combat and Game.combat.status and Game.combat.status() ~= Status.STANDING then
		return
	end
		...
		for teamId, teamPings in pairs(gPings) do
			for pingIndex = #teamPings, 1, -1 do
				...
				if distance2d <= cfgPing.renderDist then
					local spriteName = ping.targetPed and PING_SPRITE_ENEMY
						or (ping.color and ping.color.sprite or "azul")
					...
					if onScreen then
						...
						drawWorldText(
							ping.coords,
							math.ceil(#(camCoords - ping.coords)) .. "m",
							distance2d,
							{ zDiv = 250, scaleMin = 0.14, scaleMax = 0.28, scaleDist = 280, alpha = pingAlpha }
						)

						renderPingIcon(ping.coords + vector3(0.0, 0.0, distance2d / 100), spriteName)
					end
				end
...
Game.session:onNet("team.setup", function(namesData)
	if not Game.session.names then
		Game.session.names = {}
	end
...
Game.session:onNet("team.states", function(statesData)
	for src, info in pairs(statesData) do
		local entry = gSquadInfo[src]

		if not entry then
			entry = {}
			gSquadInfo[src] = entry
		end

		entry.health = info.health
		entry.armor = info.armor
	end
end)
...
Game.session:onNet("ping.show", function(coords, teamServerId, targetServerId)
	...
	if #teamPings >= 1 then
		table.sort(teamPings, function(a, b)
