...
function Totem:nearestCard()
	...
	for id, c in pairs(cards) do
		if #(origin - vec3(c.x, c.y, c.z)) <= CARD_RANGE then
			return id
		end
	end
end
...
function Totem:nearestPlatform()
	...
	for i = 1, #validSlots do
		...
		if p and #(origin - vec3(p[1], p[2], p[3])) <= INTERACT_RANGE then
			return idx
		end
	end
end
...
Game.session:onNet("revive.cardPickedUp", function(cardId, totalCards)
	...
	Game.ui.send(
		"hud:safezone",
		{ visible = true, title = "CARD", message = ("Card coletado! (%d)"):format(totalCards) }
	)
...
Game.session:listen("interact.pressed", function()
	...
	if channeling or collectedCount <= 0 then
		return
	end
	local idx = Totem:nearestPlatform()
	if not idx then
