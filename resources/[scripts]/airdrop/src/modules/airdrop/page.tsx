...
import iconAirdrop from '@/assets/icon-airdrop.svg'
...
export default function Airdrop() {
  ...
  // Backend sends pure data (id/title/description); icons are resolved locally.
  ...
  const select = useNuiMutation<{ success?: boolean }, { id: string }>({
    event: 'selectAbility',
    onSuccess: () => close(),
    // If the backend rejects the pick, release the selection so it can retry.
  ...
  const handleSelect = (ability: AbilityData) => {
    ...
    // No Lua to answer in the browser — close after the pick animation.
