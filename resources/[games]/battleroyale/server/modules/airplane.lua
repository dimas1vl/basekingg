...
local function startAirplane(match)
	...
	log(
		"info",
		("match %d: airplane phase (from=%.0f,%.0f to=%.0f,%.0f duration=%.1fs)"):format(
			match.id,
			from.x,
			from.y,
			to.x,
	...
	CreateThread(function()
		while match.state == MatchState.WAITING do
			...
			if not countdownStarted and (playerCount >= 30 or aliveSquads >= 10) then
				...
				log(
					"info",
					("match %d: countdown started (players=%d squads=%d)"):format(match.id, playerCount, aliveSquads)
				)
			end
...
GM:registerNetEvent("airplane.jumped", function(match, src)
	...
	if not airplane or airplane.finished then
		return
	end

	if airplane.jumped[src] then
		return
	end
	...
	log(
		"info",
		("match %d: player %d jumped from airplane (%d/%d)"):format(
			match.id,
			src,
			airplane.jumpedCount,
			#match.playerList
		)
	)
...
GM:registerNetEvent("airplane.leaderJump", function(match, src, heading)
	...
	log(
		"info",
		("match %d: squad %d leader %d jumped (heading=%.1f)"):format(match.id, squadIndex, src, heading or 0.0)
	)
end)
