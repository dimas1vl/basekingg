...
local function isInteractionAllowed()
    ...
    if Core._busy and Core._busy() then return false end
...
local function findClosestPickup()
    ...
    for idx, p in pairs(pickups) do
        ...
        if not p.entity or not DoesEntityExist(p.entity) then goto next end
...
function Loot:updatePickupEffects()
    ...
    for _, p in pairs(pickups) do
        ...
        if p.entity and DoesEntityExist(p.entity) then
            local heading = (GetEntityHeading(p.entity) + 1.0) % 360.0
...
Game.session:onNet('chest.seed', function(seed)
    ...
    local count = 0
    for _ in pairs(allChests) do count = count + 1 end
...
Game.prompts.register({
    ...
    available = function()
        ...
        if opening then return false end
        if Game.session:currentPhase() ~= MatchState.STARTED then return false end
        if GetGameTimer() < pickupCooldown then return false end
...
Game.session:listen('interact.pressed', function()
    ...
    if not isInteractionAllowed() then return end
    ...
    if not chest then
        ...
        if pickup and GetGameTimer() >= pickupCooldown then
    ...
    local cancel = Game.ui.holdAction({
        label = 'ABRINDO BAU',
        key = 'E',
...
RegisterCommand('bugcaixa', function()
    ...
    for _ in pairs(allChests) do allCount = allCount + 1 end
    for _ in pairs(visibleChests) do visCount = visCount + 1 end

    print('==================== [bugcaixa] ====================')

    print(('[bugcaixa] session.active=%s | phase=%s (esperado=%s)'):
        format(tostring(Game.session:active()), tostring(Game.session:currentPhase()), tostring(MatchState.STARTED)))

    print(('[bugcaixa] spawnPoints=%d | allChests=%d | visibleChests=%d'):
        format(spawnPoints and #spawnPoints or -1, allCount, visCount))

    print(('[bugcaixa] cfg: CHEST_LOD=%s CHEST_INTERACT_RANGE=%s chestSpawnChance=%s'):
        format(tostring(CHEST_LOD), tostring(CHEST_INTERACT_RANGE), tostring(Config.BR.loot.chestSpawnChance)))

    print(('[bugcaixa] flags: opening=%s Core._busy=%s inVehicle=%s isInteractionAllowed=%s'):
        format(tostring(opening), tostring(Core._busy and Core._busy() or nil),
            tostring(IsPedInAnyVehicle(ped, false)), tostring(isInteractionAllowed())))
    ...
    print(('[bugcaixa] near chest: idx=%s type=%s pos=%.2f,%.2f,%.2f'):
        format(tostring(bestIdx), tostring(bestChest.chestType),
            bestChest.pos.x, bestChest.pos.y, bestChest.pos.z))

    print(('[bugcaixa] dist ped->pos: 3D=%.3f horiz=%.3f dz=%.3f | dist cam->pos=%.3f'):
        format(bestDist, horiz, dz, dCam))

    print(('[bugcaixa] visibleChests=%s | ent=%s | model=%s (hash=%s)'):
        format(tostring(visibleChests[bestIdx] ~= nil), tostring(entityExists),
            tostring(modelLoaded), tostring(modelHash)))

    if entPos then
        print(('[bugcaixa] ent pos=%.2f,%.2f,%.2f | Z ent - Z chest.pos = %.3f'):
            format(entPos.x, entPos.y, entPos.z, zDiff))
    end

    local fcIdx, fcChest = findClosestChest()
    print(('[bugcaixa] findClosestChest: idx=%s = %s'):
        format(tostring(fcIdx), tostring(fcChest ~= nil)))
    ...
    elseif not inRange then
        if zDiff and math.abs(zDiff) > 1.0 then
            verdict = ('d3d (%.3f > %s):: Z ( Z=%.2f vs pos Z=%.2f) [613]'):
                format(bestDist, tostring(CHEST_INTERACT_RANGE), entPos and entPos.z or 0.0, bestChest.pos.z)
        else
            verdict = ('d3d > range (%.3f > %s)'):
                format(bestDist, tostring(CHEST_INTERACT_RANGE))
        end
    else
        verdict = 'certo'
    end
...
RegisterCommand('bugarma', function()
    ...
    local pickupCount = 0
    for _ in pairs(pickups) do pickupCount = pickupCount + 1 end

    print('==================== [bugarma] ====================')

    print(('[bugarma] session.active=%s | phase=%s (esperado=%s)'):
        format(tostring(Game.session:active()), tostring(Game.session:currentPhase()), tostring(MatchState.STARTED)))

    print(('[bugarma] pickups=%d | cfg: PICKUP_LOD=%s PICKUP_INTERACT_RANGE=%s PICKUP_COOLDOWN_MS=%s'):
        format(pickupCount, tostring(PICKUP_LOD), tostring(PICKUP_INTERACT_RANGE), tostring(PICKUP_COOLDOWN_MS)))

    local cooldownRemaining = math.max(0, pickupCooldown - GetGameTimer())
    print(('[bugarma] flags: opening=%s Core._busy=%s inVehicle=%s cooldownRestante=%dms'):
        format(tostring(opening), tostring(Core._busy and Core._busy() or nil),
            tostring(IsPedInAnyVehicle(ped, false)), cooldownRemaining))
    ...
    print(('[bugarma] near pickup: idx=%s model=%s itemName=%s'):
        format(tostring(bestIdx), tostring(bestPickup.model), tostring(itemName)))

    print(('[bugarma] pos=%.2f,%.2f,%.2f | dist ped->pos: 3D=%.3f horiz=%.3f dz=%.3f | dist cam->pos=%.3f'):
        format(bestPickup.pos.x, bestPickup.pos.y, bestPickup.pos.z, bestDist, horiz, dz, dCam))

    print(('[bugarma] entidade=%s | modelLoaded=%s | entPos=%s | Z ent - Z pos = %s | dist ped->ent=%s'):
        format(tostring(entityExists), tostring(HasModelLoaded(bestPickup.model)),
            entPos and ('%.2f,%.2f,%.2f'):format(entPos.x, entPos.y, entPos.z) or 'nil',
            tostring(zDiff), tostring(dEnt and ('%.3f'):format(dEnt) or 'nil')))

    print(('[bugarma] getFreeSpace(%s)=%s | itemDef=%s'):
        format(tostring(itemName), tostring(freeSpace), tostring(itemDef and itemDef.name or nil)))

    local fcIdx, fcPickup = findClosestPickup()
    print(('[bugarma] findClosestPickup: idx=%s = %s'):
        format(tostring(fcIdx), tostring(fcPickup ~= nil)))
    ...
    elseif not inRangeEnt then
        if zDiff and math.abs(zDiff) > 1.0 then
            verdict = ('BLOQUEADO: dist ped->ent > range (%.3f > %s) por divergência de Z (ent Z=%.2f vs pos Z=%.2f)'):
                format(dEnt, tostring(PICKUP_INTERACT_RANGE), entPos and entPos.z or 0.0, bestPickup.pos.z)
        else
    ...
    elseif freeSpace == nil or freeSpace <= 0 then
        verdict = ('BLOQUEADO: sem espaço no inventário para %s (freeSpace=%s)'):format(tostring(itemName), tostring(freeSpace))
    else
