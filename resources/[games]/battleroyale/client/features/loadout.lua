...
function getFreeSpace(itemName)
	...
	for _, data in invSlots() do
		if not data then
			slots = slots + 1
		elseif data.index == itemName then
			total = total + data.amount
		end
	end
	if slots == 0 and total == 0 then
		return 0
	end
...
function giveItem(itemName, amount, ammo)
	if amount <= 0 or not GItems[itemName] then
		return false
	end
...
local function getItemByHash(hash)
	for _, v in pairs(gInventory) do
		if GetHashKey(v.index) == hash then
			return v
		end
	end
end
...
function updateInventory()
	...
	if lastInvSync > GetGameTimer() then
		return
	end
...
local function putWeaponHands(weaponName)
	...
	if not IsPedReloading(ped) then
		if weaponCooldown > GetGameTimer() then
			return
		end
	end
	weaponCooldown = GetGameTimer() + 450

	if weaponName == gCurrentWeapon then
		return removeWeapon()
	end
	removeWeapon()
	...
	Citizen.CreateThread(function()
		...
		while Game.session:active() do
			if gCurrentWeapon ~= weaponName then
				break
			end
			SetPlayerCanDoDriveBy(PlayerId(), true)
			if Core.prone() or Core.layingDown() then
				return removeWeapon()
			end
			...
			if inVeh ~= dbgPrevInVeh or equipped ~= dbgPrevEquipped then
				print(
					("[loadout] state %s | inVeh=%s equipped=%s weapon=%s clip=%s ammoCache=%s invAmmo=%s"):format(
						weaponName,
						tostring(inVeh),
						tostring(equipped),
						tostring(weaponHash),
						tostring(clip),
						tostring(ammoCache[weaponHash]),
						tostring(getItemAmount(itemAmmo))
					)
				)
				dbgPrevInVeh = inVeh
			...
			if equipped == weaponHash and not inVeh and clip < ammoCache[weaponHash] then
				...
				print(
					("[loadout] DEDUCT %s | spent=%d clip=%d->%d ammoCache=%d invAmmo=%d->%d invVeh=%s"):format(
						weaponName,
						spent,
						ammoCache[weaponHash] + spent,
						clip,
						ammoCache[weaponHash],
						beforeInv,
						getItemAmount(itemAmmo),
						tostring(inVeh)
					)
				)
			end
			...
			if grenadeItem then
				...
				if inVeh then
					if HasPedGotWeapon(ped, gh) then
						RemoveWeaponFromPed(ped, gh)
					end
			...
			if isGrenadeGrp and not inVeh then
				...
				if eqItem and GItems[eqItem.index].type == "GRENADE" then
					...
					while GetSelectedPedWeapon(ped) ~= weaponHash do
			...
			if inVeh then
				if not isVehWeapon then
					...
					if ammoCache[weaponHash] ~= minA then
						ammoCache[weaponHash] = minA
					end
				elseif clip ~= 0 then
					if clip ~= ammoCache[weaponHash] then
						ammoCache[weaponHash] = clip
					end
				end
...
local function putThrowablesHands(weaponName)
	...
	if not IsPedReloading(ped) then
		if weaponCooldown > GetGameTimer() or IsPedInAnyVehicle(ped) or Core.prone() or Core.layingDown() then
			return
		end
	end
	weaponCooldown = GetGameTimer() + 450
	...
	Citizen.CreateThread(function()
		while Game.session:active() do
			...
			if Core.prone() or IsPedInAnyVehicle(ped) or Core.layingDown() then
				return removeWeapon()
			end
			if IsPedShooting(ped) then
				...
				SendNUIMessage({
					action = "hud:weapon",
					data = { index = weaponName, ammo = { current = 1, max = newAmt } },
				})
				if newAmt <= 0 then
					return removeWeapon()
				end
...
local ITEM_ACTIONS = {
	ARMOUR_STANDARD = function()
		...
		if GetPedArmour(PlayerPedId()) >= cfgInv.armourCap then
			return
		end
		armourActive = true
	...
	HEALTH_STANDARD = function()
		...
		if health >= cfgInv.maxHealth or health <= 100 then
			return
		end
	...
	WEAPON_SMOKEGRENADE = function()
		putThrowablesHands("WEAPON_SMOKEGRENADE")
	end,
	WEAPON_ASSAULTSMG = function()
		putWeaponHands("WEAPON_ASSAULTSMG")
	end,
	WEAPON_MICROSMG = function()
		putWeaponHands("WEAPON_MICROSMG")
	end,
	WEAPON_MACHINEPISTOL = function()
		putWeaponHands("WEAPON_MACHINEPISTOL")
	end,
	WEAPON_APPISTOL = function()
		putWeaponHands("WEAPON_APPISTOL")
	end,
	WEAPON_PUMPSHOTGUN = function()
		putWeaponHands("WEAPON_PUMPSHOTGUN")
	end,
	WEAPON_PISTOL_MK2 = function()
		putWeaponHands("WEAPON_PISTOL_MK2")
	end,
	WEAPON_ASSAULTRIFLE = function()
		putWeaponHands("WEAPON_ASSAULTRIFLE")
	end,
	WEAPON_SPECIALCARBINE = function()
		putWeaponHands("WEAPON_SPECIALCARBINE")
	end,
	WEAPON_CARBINERIFLE = function()
		putWeaponHands("WEAPON_CARBINERIFLE")
	end,
}
...
AddEventHandler("inventory:moveItem", function(data)
	...
	local ped = PlayerPedId()
	if IsPedInAnyVehicle(ped) or IsPedClimbing(ped) then
		return
	end
	if gInventory[slot] then
		if amount > gInventory[slot].amount then
			amount = gInventory[slot].amount
		end
		local idx = gInventory[slot].index
		if idx == "PARACHUTE" then
			return
		end
...
RegisterCommand("openBackpack", function()
	if gActiveItem or IsPauseMenuActive() then
		return
	end
	if Game.session:currentPhase() ~= MatchState.STARTED then
		return
	end
	if Game.combat.status() ~= Status.STANDING then
		return
	end
...
RegisterCommand("kingg:cancel", function()
	...
	SetTimeout(Config.BR.inventory.cancelCooldown, function()
		cancelling = false
	end)
end)
...
RegisterCommand("kingg:bind", function(_, args)
	local slot = args[1]
	if invOpen or gActiveItem or IsPauseMenuActive() then
		return
	end
...
RegisterCommand("inventory:drop.weapon", function()
	...
	if currentHash == GetHashKey("WEAPON_UNARMED") or currentHash == GetHashKey("WEAPON_KNIFE") then
		return
	end
	local itemIndex = nil
	for k, _ in pairs(GItems) do
		if GetHashKey(k) == currentHash then
			itemIndex = k
		end
	end
...
Game.session:onNet("smoke.create", function(coords)
	...
	Citizen.CreateThread(function()
		...
		while DoesEntityExist(obj) do
			...
			if not fading and el >= DURATION then
				fading = true
