...
function isCoordsSafezone(x, y)
    ...
    if not state or not state.active or not state.gas then return true end
...
function Zone:updateRadarZoom()
    if totalStages <= 3 then return end
...
RegisterNetEvent('kingg:safezone:start', function(data)
    print(('[zone] kingg:safezone:start received — damage=%s phase=%s'):format(tostring(data.damage), tostring(data.phase)))
    if data.gas then
    ...
    exports['kingg']:setSafezoneHandlers({
        onOut = function()
            if winCelebration then return end
            outsideZone = true
        ...
        onDamageTick = function(baseDamage)
            ...
            if winCelebration or GetEntityHealth(ped) <= 0 then return end
...
Game.session:onNet('safezone.countdown', function(phase, displayTime, totalPh)
    print(('[zone] safezone.countdown received — phase=%s displayTime=%s totalPh=%s'):format(tostring(phase), tostring(displayTime), tostring(totalPh)))
    currentStage = phase
    phaseGen = phaseGen + 1
    if totalPh then totalStages = totalPh end
    local gen = phaseGen
