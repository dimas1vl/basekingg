...
local function ensurePtfx(dict)
    ...
    if ptfxLoaded[dict] then return true end
...
local function registerPrompt()
    ...
    Game.prompts.register({
        ...
        available = function()
            ...
            if not dropLanded then return false end

            if activeDropId == nil then return false end

            if openingMe then return false end
        ...
        position = function()
            ...
            if not dropPos then return nil end
...
Game.session:onNet('airdrop.spawn', function(dropId, x, y, z, startHeight)
    ...
    CreateThread(function()
        ...
        if activeDropId ~= dropId then return end
        ...
        if not dropEntity or dropEntity == 0 then return end
        ...
        if chuteHash then
            ...
            if parachuteEntity and parachuteEntity ~= 0 then
                ...
                AttachEntityToEntity(
                    parachuteEntity, dropEntity, 0,
                    PARACHUTE_OFFSET.x, PARACHUTE_OFFSET.y, PARACHUTE_OFFSET.z,
                    0.0, 0.0, 0.0,
        ...
        if chuteHash then SetModelAsNoLongerNeeded(chuteHash) end

        if ensurePtfx(PTFX_DICT) then

            UseParticleFxAssetNextCall(PTFX_DICT)
            flareFxId = StartParticleFxLoopedAtCoord(PTFX_FLARE, x, y, targetZ - 0.99, 0.0, 0.0, 0.0, 2.0, false, false, false, false)
        end
...
Game.session:listen('interact.pressed', function()
    ...
    if not dropLanded then return end

    if activeDropId == nil then return end

    if openingMe then return end

    if not dropPos then return end

    local dist = #(GetEntityCoords(PlayerPedId()) - dropPos)

    if dist > cfgDrop.collectRange then return end
    ...
    cancelHold = Game.ui.holdAction({
        ...
        check = function()
            ...
            if activeDropId ~= currentDropId then return true end

            if not dropPos then return true end
...
Game.session:listen('interact.released', function()
    ...
    if not openingMe then return end
...
Game.session:onNet('airdrop.remove', function(dropId)
    ...
    if dropPos and ensurePtfx(PTFX_COLLECT) and dropPos.x then
        ...
        StartParticleFxNonLoopedAtCoord(PTFX_COLLECT_NAME, dropPos.x, dropPos.y, dropPos.z + 1.0, 0.0, 0.0, 0.0, 2.0, false, false, false)
    end
