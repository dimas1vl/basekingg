...
print(('^2[dom-sv] SHARED_BUCKET=%d (+%d buckets ocultos), script pronto.^7'):format(SHARED_BUCKET, #(Config.Domination.locations or {})))

Sql = {}
function Sql.single(q, p)  return exports['oxmysql']:singleSync(q, p) end
function Sql.query(q, p)   return exports['oxmysql']:querySync(q, p) end
function Sql.execute(q, p) return exports['oxmysql']:executeSync(q, p) end
function Sql.insert(q, p)  return exports['oxmysql']:insertSync(q, p) end
...
DDL = {
    ...
    domination_team_members = [[
        CREATE TABLE IF NOT EXISTS `domination_team_members` (
            ...
            `role`      ENUM('lider','gerente','sublider','recrutador','membro') NOT NULL DEFAULT 'membro' COLLATE 'utf8mb4_uca1400_ai_ci',
...
CreateThread(function()
    ...
    local order = {
        'domination_progress', 'domination_weapons', 'domination_loadout',
        'domination_vehicles', 'domination_vehicle_favorites',
        'domination_teams', 'domination_team_members',
    }
