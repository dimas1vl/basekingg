...
interface DmMapInfoLocal extends DmMapInfo {
  receivedAtMs: number
}

interface DmMapResult {
  mapName: string
  nextMapName: string
  leaderSrc?: number | null
...
function MapBanner({ info }: { info: DmMapInfoLocal | null }) {
  ...
  const elapsed = now - info.receivedAtMs
  const remaining = Math.max(0, info.remainingMs - elapsed)
  const isWarning = remaining > 0 && remaining < 60_000

  return (
    <div className="absolute top-[24px] left-1/2 -translate-x-1/2 pointer-events-none flex items-center gap-[1rem]">
      <span
        className="flex items-center justify-center h-[2.4rem] px-[1rem] font-['Termina:Bold',sans-serif] text-[1.1rem] tracking-[0.3rem]"
        style={{ backgroundColor: '#c8fe4e', color: '#1d1c26' }}
      >
        MAPA {info.index}/{info.total}
      </span>
      <span
        className="font-['Termina:Bold',sans-serif] text-[1.4rem] tracking-[0.35rem]"
        style={{ color: '#f8efff' }}
      >
        {info.name}
      </span>
      <span
        className="flex items-center justify-center h-[2.4rem] px-[1rem] font-['Termina:Bold',sans-serif] text-[1.4rem] tracking-[0.25rem] tabular-nums"
        style={{
          backgroundColor: 'rgba(15,12,24,0.92)',
          color: isWarning ? '#ff5858' : '#f8efff',
          border: `1px solid ${isWarning ? '#ff5858' : 'rgba(248,239,255,0.15)'}`,
        }}
      >
...
function MapResultOverlay({
  result,
  selfSrc,
  visible,
}: {
  result: DmMapResult | null
  selfSrc: number
  visible: boolean
}) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none"
      style={{ background: 'rgba(8,6,14,0.55)' }}
    >
      <div
        className="w-[680px] overflow-hidden"
        style={{
      ...
      >
        <div
          className="px-[2.4rem] py-[2rem] text-center"
          style={{
            background: 'linear-gradient(90deg, rgba(157,13,233,0.18) 0%, rgba(0,0,0,0) 100%)',
            borderBottom: '1px solid rgba(248,239,255,0.1)',
          }}
        >
          <div
            className="font-['Termina:Medium',sans-serif] text-[1rem] tracking-[0.3rem] opacity-55 text-[#f8efff] mb-[0.4rem]"
          >
            FIM DO MAPA
          </div>
          <h1
            className="font-['Termina:Bold',sans-serif] text-[2.4rem] tracking-[0.4rem]"
            style={{ color: '#c8fe4e' }}
          >
            {result.mapName}
          </h1>
          ...
          {result.leaderName ? (
            <div className="mt-[1.4rem]">
              <div className="font-['Termina:Medium',sans-serif] text-[0.95rem] tracking-[0.3rem] opacity-55 text-[#f8efff]">
                VENCEDOR
              </div>
              <div className="mt-[0.4rem] flex items-center justify-center gap-[0.8rem]">
                <span className="font-['Termina:Bold',sans-serif] text-[1.8rem] tracking-[0.2rem] text-[#f8efff]">
                  {result.leaderName}
                </span>
                <span
                  className="flex items-center justify-center h-[2rem] px-[0.8rem] font-['Termina:Bold',sans-serif] text-[1rem]"
                  style={{ backgroundColor: '#c8fe4e', color: '#1d1c26' }}
                >
                  {result.leaderKills ?? 0} KILLS
                </span>
              </div>
            </div>
          ) : (
            <div className="mt-[1.4rem] font-['Termina:Medium',sans-serif] text-[1rem] tracking-[0.3rem] opacity-55 text-[#f8efff]">
              SEM KILLS REGISTRADAS
            </div>
          )}
        </div>

        <div
          className="px-[2.4rem] py-[1.6rem] text-center"
          style={{ borderBottom: '1px solid rgba(248,239,255,0.06)' }}
        >
          <div className="font-['Termina:Medium',sans-serif] text-[0.9rem] tracking-[0.25rem] opacity-45 text-[#f8efff]">
            PROXIMO MAPA
          </div>
          <div className="mt-[0.4rem] font-['Termina:Bold',sans-serif] text-[1.6rem] tracking-[0.3rem] text-[#f8efff]">
            {result.nextMapName}
          </div>
        </div>

        <div className="max-h-[40vh] overflow-y-auto">
          {(result.scoreboard || []).slice(0, 5).map((r, i) => {
            const isSelf = r.src === selfSrc
            return (
              <div
                key={r.src}
                className="grid grid-cols-[3rem_1fr_4.5rem_4.5rem] gap-3 px-[2.4rem] py-[0.6rem] text-[1.1rem]"
                style={{
                  color: isSelf ? '#c8fe4e' : '#f8efff',
              ...
              >
                <span className="opacity-55 font-['Termina:Medium',sans-serif]">{i + 1}</span>
                <span className="truncate font-['Termina:Demi',sans-serif]">{r.name}</span>
                <span className="text-right font-['Termina:Bold',sans-serif]">{r.kills}</span>
                <span className="text-right opacity-65 font-['Termina:Medium',sans-serif]">{r.deaths}</span>
              </div>
            )
          })}
        </div>
...
export default function Home() {
  ...
  const [mapInfo, setMapInfo] = useState<DmMapInfoLocal | null>(null)
  const [mapResult, setMapResult] = useState<DmMapResult | null>(null)
  const [mapResultVisible, setMapResultVisible] = useState(false)
  ...
  useListener<{ data: DmMapInfo }>('mapInfo', ({ data }) => {
    if (!data) return
    setMapInfo({ ...data, receivedAtMs: Date.now() })
  })

  useListener<{ data: DmMapResult; selfSrc?: number }>('mapResult', ({ data, selfSrc: sSrc }) => {
    if (!data) return
    if (typeof sSrc === 'number') setSelfSrc(sSrc)
    setMapResult(data)
    setMapResultVisible(true)
    setTimeout(() => setMapResultVisible(false), data.showMs || 6000)
  })
  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      <MapBanner info={mapInfo} />
      <Scoreboard rows={scoreboardRows} selfSrc={selfSrc} visible={scoreboardVisible} />
      <MapResultOverlay result={mapResult} selfSrc={selfSrc} visible={mapResultVisible} />
