...
import cardBgHover from '@/assets/card-bg-hover.svg'
...
export function AbilityCard({ ability, selected, disabled, onSelect }: AbilityCardProps) {
  // The hover/selected look (purple border + bigger icon + glow) is driven by
  // the `active` flag so it can be triggered by mouse hover or a confirmed pick.
  ...
  return (
    ...
    >
      {/* Default torn-crate background (faint white edge) */}
      <img
        src={cardBg}
        alt=""
        className={cn(
      ...
      {/* Hover/selected background (solid purple edge + glow) */}
      <img
        src={cardBgHover}
        alt=""
        className={cn(
      ...
      {/* Icon — grows ~1.16x on hover/selected, anchored to its center */}
