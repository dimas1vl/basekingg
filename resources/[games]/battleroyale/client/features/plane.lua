...
local function calculateSpeed(ped)
	local coords = GetEntityCoords(ped)
	local now = GetGameTimer()

	if not lastSpeedCoords then
		lastSpeedCoords = coords
		lastSpeedTime = now
		return 0.0
	end

	local dt = (now - lastSpeedTime) / 1000.0
	local speed = dt > 0.0 and (#(coords - lastSpeedCoords) / dt) or 0.0
...
local function setPlayerFollowing(id)
	...
	local name = (Game.session.names and Game.session.names[id])
		or ((playerIdx ~= -1) and GetPlayerName(playerIdx))
		or tostring(id)
...
local function swapFollowPlayer(value)
	...
	if phase ~= MatchState.STARTED and phase ~= MatchState.AIRPLANE then
		return
	end

	local mySrc = GetPlayerServerId(PlayerId())

	if phase == MatchState.STARTED and playersJumped[mySrc] then
		return
	end

	local now = GetGameTimer()

	if now - cooldownFollow < 250 then
		return
	end
...
Game.session:onNet("airplane.playerJumped", function(id)
	...
	if id ~= playerFollowing or playersJumped[mySrc] then
		return
	end
	...
	CreateThread(function()
		while flightActive and capturedVeh and DoesEntityExist(capturedVeh) do
			...
			if inVehicle then
				...
				Game.ui.send(
					"hud:interaction",
					{ visible = false, key = "", action = "", detail = "", detailValue = "" }
				)
			end
...
Game.session:onNet("airplane.leaderJumped", function(srcLeader, heading)
	...
	if srcLeader == GetPlayerServerId(PlayerId()) then
		return
	end
