...
local function computeIncrementalZone(match, phase)
    ...
    if isFinalPhase(phase) then
        ...
        log('info', ('match %d: safezone phase %d (final/gas) center=%.0f,%.0f r=%.0f'):format(
            match.id, phase, zone.center.x, zone.center.y, zone.radius
        ))
    else
        zone = Core.getNextSafeZone(prevZone, preset.radius, match.bucket, opts)
        log('info', ('match %d: safezone phase %d (dynamic) center=%.0f,%.0f r=%.0f'):format(
            match.id, phase, zone.center.x, zone.center.y, zone.radius
        ))
    end
...
local function showInitialZone(match)
    ...
    if not szData then return end

    local zones = szData.zones
    local first = zones[1]
    local second = zones[2]

    print(('[safezone] showInitialZone: match %d — gas(%.0f,%.0f r=%.0f) safe=%s playerCount=%d'):format(
        match.id, first.center.x, first.center.y, first.radius,
    ...
    emitSafezoneEvent(match, 'kingg:safezone:start', {
        ...
        safe = second and { x = second.center.x, y = second.center.y, radius = second.radius } or nil,
        damage = 0,
        phase = 1,
    })
...
local function startPhaseProgression(match)
    ...
    CreateThread(function()
        ...
        for phase = 2, #zones do
            ...
            emitSafezoneEvent(match, 'kingg:safezone:phase', phase, currentZone.damage,
                { x = currentZone.center.x, y = currentZone.center.y, radius = currentZone.radius },
                nextZone and nextZone.center and { x = nextZone.center.x, y = nextZone.center.y, radius = nextZone.radius } or nil
            )
            ...
            log('info', ('match %d: safezone phase %d (radius=%.0f, damage=%d, shrink=%ds)'):format(
                match.id, phase, currentZone.radius, currentZone.damage, currentZone.shrinkTime
            ))
...
GM:registerNetEvent('safezone.useRadar', function(match, src)
    ...
    if nextPhase > #zones then return end
    ...
    if not nextZone or not nextZone.center then return end
