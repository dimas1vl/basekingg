...
function getKillFeedWeaponSrc(image?: string, weapon?: string): string {
  const rawImage = image?.trim()
  if (rawImage) {
    if (rawImage.includes('/') || rawImage.includes('\\') || rawImage.includes('.')) {
      return rawImage
    }
    return `${A}${normalizeImageKey(rawImage)}.png`
  }

  if (weapon) {
    return `${A}${normalizeImageKey(weapon)}.png`
  }
...
export default function KillFeed({ entries }: KillFeedProps) {
  return (
    <div className="absolute content-stretch flex flex-col gap-[6px] items-start right-[33px] top-[42px] w-[359px]">
      {entries.map((entry) => {
              ...
              <div className="content-stretch flex gap-[12px] items-center justify-end relative shrink-0">
                ...
                <div className="h-[10.794px] relative shrink-0 w-[31.934px]">
                  <img
                    ...
                    src={getKillFeedWeaponSrc(entry.weaponImage, entry.weapon)}
                    onError={(event) => {
